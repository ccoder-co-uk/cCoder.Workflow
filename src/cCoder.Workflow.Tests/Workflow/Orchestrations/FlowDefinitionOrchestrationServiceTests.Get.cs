...
public partial class FlowDefinitionOrchestrationServiceTests
{
    ...
    public void ShouldReturnProcessingResultWhenGet()
    {
        ...
        flowDefinitionProcessingServiceMock.Setup(expression: x => x.Get(id: id)).Returns(value: entity);

        // When
        FlowDefinition result = orchestrationService.Get(id: id);

        // Then
        result.Should().BeSameAs(expected: entity);
        flowDefinitionProcessingServiceMock.Verify(expression: x => x.Get(id: id), times: Times.Once);
