...
public partial class CalendarEventOrchestrationServiceTests
{
    ...
    public void ShouldReturnProcessingResultsWhenGetAll()
    {
        ...
        calendarEventProcessingServiceMock.Setup(expression: x => x.GetAll(ignoreFilters: true))
            .Returns(value: entities);

        // When
        IQueryable<CalendarEvent> result = orchestrationService.GetAll(ignoreFilters: true);

        // Then
        result.Should()
            .BeSameAs(expected: entities);

        calendarEventProcessingServiceMock.Verify(expression: x => x.GetAll(ignoreFilters: true), times: Times.Once);
