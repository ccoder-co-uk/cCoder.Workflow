    ...
    public string GetHome() =>
        TryCatch(operation: () =>
        {
            ...
            [
                ...
                string.Empty,
                "Hosted services:",
                $"- InstanceMaintenanceBackgroundService: deletes workflow instances older than {instanceMaxAgeDays} day(s).",
                $"- QueueInstanceBackgroundService: resets executing workflow instances older than {executingTimeoutMinutes} minute(s) back to Queued.",
                $"- ScheduledTaskRunnerBackgroundService: checks scheduled tasks every {scheduledTaskIntervalMinutes} minute(s) and raises scheduled_task_execute events for due tasks.",
                string.Empty,
                "Event listeners:",
