...
{
    [Fact]
    public async Task ShouldMapAndCallBrokerWhenRaiseCalendarEventDeleteEventAsync()
    {
        // Given
        ...
        calendarEventEventBrokerMock
            .Setup(expression: x =>
                x.RaiseCalendarEventDeleteEventAsync(message: It.IsAny<EventMessage<CalendarEvent>>())
            )
...
expression: x => x.RaiseCalendarEventDeleteEventAsync(message: It.IsAny<EventMessage<CalendarEvent>>()),
times: Times.Once
        );
