...
public partial class ScheduledTaskEventServiceTests
{
    ...
    public async Task ShouldMapAndCallBrokerWhenRaiseScheduledTaskUpdateEventAsync()
    {
        ...
        scheduledTaskEventBrokerMock
            .Setup(expression: x =>
                x.RaiseScheduledTaskUpdateEventAsync(message: It.IsAny<EventMessage<ScheduledTask>>())
            )
...
expression: x => x.RaiseScheduledTaskUpdateEventAsync(message: It.IsAny<EventMessage<ScheduledTask>>()),
times: Times.Once
        );
