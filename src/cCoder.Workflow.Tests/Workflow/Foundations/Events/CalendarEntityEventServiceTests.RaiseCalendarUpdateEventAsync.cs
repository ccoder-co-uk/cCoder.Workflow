...
public partial class CalendarEntityEventServiceTests
{
    ...
    public async Task ShouldMapAndCallBrokerWhenRaiseCalendarUpdateEventAsync()
    {
        ...
        calendarEntityEventBrokerMock
            .Setup(expression: x =>
                x.RaiseCalendarUpdateEventAsync(message: It.IsAny<EventMessage<Calendar>>())
            )
...
expression: x => x.RaiseCalendarUpdateEventAsync(message: It.IsAny<EventMessage<Calendar>>()),
times: Times.Once
        );
