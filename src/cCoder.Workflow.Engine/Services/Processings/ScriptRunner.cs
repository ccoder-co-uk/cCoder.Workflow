...
arg2: $"{exception.Message}{Environment.NewLine}Context: {exception.Source}{Environment.NewLine}Target: {target}{Environment.NewLine}{string.Join(separator: Environment.NewLine, values: context)}");
        ...
        catch (Microsoft.CodeAnalysis.Scripting.CompilationErrorException exception)
        {
            log?.Invoke(
arg1: WorkflowLogLevel.Error,
arg2: $"Compilation failed:{Environment.NewLine}{exception.Message}{Environment.NewLine}{string.Join(separator: Environment.NewLine, values: exception.Diagnostics)}");
    ...
    private ScriptOptions BuildOptions(string[] imports) =>
        ScriptOptions.Default
            .AddReferences(references: ResolveReferences(imports: imports))
            .WithImports(imports: imports);
