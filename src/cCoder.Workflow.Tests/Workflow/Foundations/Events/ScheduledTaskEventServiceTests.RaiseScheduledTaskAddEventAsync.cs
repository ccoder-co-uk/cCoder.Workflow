...
{
    [Fact]
    public async Task ShouldMapAndCallBrokerWhenRaiseScheduledTaskAddEventAsync()
    {
        // Given
        ...
        scheduledTaskEventBrokerMock
            .Setup(expression: x =>
                x.RaiseScheduledTaskAddEventAsync(message: It.IsAny<EventMessage<ScheduledTask>>())
            )
...
expression: x => x.RaiseScheduledTaskAddEventAsync(message: It.IsAny<EventMessage<ScheduledTask>>()),
times: Times.Once
        );
