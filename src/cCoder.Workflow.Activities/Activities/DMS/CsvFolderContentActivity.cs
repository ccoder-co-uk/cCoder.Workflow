...
public class CSVFolderContentActivity : DMSActivity
{
    ...
    [JsonIgnore]
    public dynamic[] ParsedData => RawData?.SelectMany(i => cCoder.Workflow.Activities.Support.Data.ParseCSV<dynamic>(i, Config)).ToArray() ?? System.Array.Empty<dynamic>();

    [JsonIgnore]
    public dynamic[] FlattenedData => ParsedData?.Select(i => cCoder.Workflow.Activities.Support.Data.Flatten(i)).ToArray() ?? System.Array.Empty<dynamic>();
