...
public partial class CalendarEventEventServiceTests
{
    ...
    public async Task ShouldMapAndCallBrokerWhenRaiseCalendarEventUpdateEventAsync()
    {
        ...
        calendarEventEventBrokerMock
            .Setup(expression: x =>
                x.RaiseCalendarEventUpdateEventAsync(message: It.IsAny<EventMessage<CalendarEvent>>())
            )
            .Callback<EventMessage<CalendarEvent>>(action: message => actualMessage = message)
            .Returns(value: ValueTask.CompletedTask);

        // When
        await service.RaiseCalendarEventUpdateEventAsync(calendarEvent: entity);
...
expression: x => x.RaiseCalendarEventUpdateEventAsync(message: It.IsAny<EventMessage<CalendarEvent>>()),
times: Times.Once
        );
