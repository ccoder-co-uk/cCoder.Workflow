...
public class XmlFolderContentActivity : DMSActivity
{
    ...
    [JsonIgnore]
    public dynamic[] ParsedData => RawData?.Select(selector:i => cCoder.Workflow.Activities.Support.Data.ParseXml<dynamic>(i)).ToArray();

    [JsonIgnore]
    public dynamic[] FlattenedData => ParsedData?.Select(selector:i => cCoder.Workflow.Activities.Support.Data.Flatten(i)).ToArray();
