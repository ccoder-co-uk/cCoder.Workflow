...
public class JsonFolderContentActivity : DMSActivity
{
    ...
    [JsonIgnore]
    public dynamic[] ParsedData => RawData?.Select(selector: i => cCoder.Workflow.Activities.Support.Data.ParseJson<dynamic>(data: i)).ToArray() ?? Array.Empty<dynamic>();

    [JsonIgnore]
    public dynamic[] FlattenedData => ParsedData?.Select(selector: i => cCoder.Workflow.Activities.Support.Data.Flatten(source: i)).ToArray() ?? Array.Empty<dynamic>();
