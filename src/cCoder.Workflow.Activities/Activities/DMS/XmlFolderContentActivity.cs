...
public class XmlFolderContentActivity : DMSActivity
{
    ...
    [JsonIgnore]
    public dynamic[] ParsedData => RawData?.Select(selector: i => cCoder.Workflow.Activities.Support.Data.ParseXml<dynamic>(data: i))
            .ToArray();
    ...
    private string Now() =>
        DateTimeOffset.UtcNow.ToString(format: "HH:mm:ss");
}