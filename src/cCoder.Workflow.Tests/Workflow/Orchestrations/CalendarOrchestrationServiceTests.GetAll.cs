...
public partial class CalendarOrchestrationServiceTests
{
    ...
    public void ShouldReturnProcessingResultsWhenGetAll()
    {
        ...
        calendarProcessingServiceMock.Setup(expression: x => x.GetAll(ignoreFilters: true))
            .Returns(value: entities);

        // When
        IQueryable<Calendar> result = orchestrationService.GetAll(ignoreFilters: true);

        // Then
        result.Should()
            .BeSameAs(expected: entities);

        calendarProcessingServiceMock.Verify(expression: x => x.GetAll(ignoreFilters: true), times: Times.Once);
