...
public class JsonFolderContentActivity : DMSActivity
{
    ...
    [JsonIgnore]
    public dynamic[] ParsedData => RawData?.Select(i => cCoder.Workflow.Activities.Support.Data.ParseJson<dynamic>(i)).ToArray() ?? Array.Empty<dynamic>();

    [JsonIgnore]
    public dynamic[] FlattenedData => ParsedData?.Select(i => cCoder.Workflow.Activities.Support.Data.Flatten(i)).ToArray() ?? Array.Empty<dynamic>();
    ...
    {
        string query = $"DocumentManagement/File?$filter=Folder/AppId eq {AppId} AND Folder/Path eq '{Path.Trim().TrimEnd("/".ToCharArray())}' AND endswith(Name, '.json')&$expand=Contents";

        if (Page != null && PageSize != null)
            query += $"&$top={PageSize}&$skip={(Page - 1) * PageSize}";

        return await api.GetODataCollection<DmsFile>(query);
    }
