...
public abstract class SftpActivity : Activity
{
    ...
    public override Task ExecuteAsync() => Task.FromResult(true);
    ...
    protected void BuildPathAsync(SftpClient client, ObjectPath folderPath)
    {
        if (folderPath.Length > 0)
        {
            ...
            if (existingFolder == null)
            {
                if (folderPath.ParentPath.Depth > 0)
                    BuildPathAsync(client, folderPath.ParentPath);
